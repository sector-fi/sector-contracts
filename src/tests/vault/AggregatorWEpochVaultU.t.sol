...
import { SectorBaseWEpochU as SectorBaseWEpoch, AggregatorWEpochVaultU as AggregatorWEpochVault, RedeemParams, DepositParams, IVaultStrategy, AuthConfig, FeeConfig } from "vaults/sectorVaults/AggregatorWEpochVaultU.sol";
...
contract AggregatorWEpochVaultUTest is SectorTest, SCYWEpochVaultUtils {
	SCYWEpochVault s1;
	SCYWEpochVault s2;
	...
	string vaultType = "SectorVaultWEpoc";
	SectorFactory factory;

	function setupFactory(string memory _vaultType) public {
		AggregatorWEpochVault vaultImp = new AggregatorWEpochVault();
	...
	function setUp() public {
		...
		bytes memory s3Data = abi.encodeWithSelector(
			AggregatorWEpochVault.initialize.selector,
			underlying,
			"SECT_VAULT",
			"SECT_VAULT",
			false,
		...
		s3 = AggregatorWEpochVault(payable(address(factory.deployVault(vaultType, s3Data))));
		...
		vault = AggregatorWEpochVault(payable(address(factory.deployVault(vaultType, vaultData))));
	...
	function testDepositRedeemNative() public {
		...
		vault = AggregatorWEpochVault(payable(address(factory.deployVault(vaultType, data))));
