...
contract AggregatorVaultUTest is SectorTest, SCYVaultUtils {
	...
	function setUp() public {
		...
		bytes memory dataS3 = abi.encodeWithSelector(
		...
		s3 = AggregatorVaultU(payable(address(factory.deployVault(vaultType, dataS3))));

		bytes memory dataVault = abi.encodeWithSelector(
			AggregatorVaultU.initialize.selector,
			underlying,
			"SECT_VAULT",
			"SECT_VAULT",
		...
		vault = AggregatorVaultU(payable(address(factory.deployVault(vaultType, dataVault))));
	...
	function sectHarvest(AggregatorVaultU _vault) public {
	...
	function sectHarvestRevert(AggregatorVaultU _vault, bytes4 err) public {
		vm.startPrank(manager);
		uint256 expectedTvl = vault.getTvl();
		uint256 maxDelta = expectedTvl / 1000; // .1%
		vm.expectRevert(err);
	...
	function sectCompleteRedeem(AggregatorVaultU _vault, address acc) public {
		vm.startPrank(acc);
		_vault.redeem();
		vm.stopPrank();
	}
