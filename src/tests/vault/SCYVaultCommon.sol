...
abstract contract SCYVaultCommon is SectorTest, SCYVaultUtils {
	SCYVault vault;
	WETH underlying;

	function setupVault(address underlying, bool takesNativeDeposit)
		public
		virtual
		returns (SCYVault);
	...
	function setUpCommonTest() public {
		underlying = new WETH();
		vault = setupVault(address(underlying), true);
	...
	function testGetBaseTokens() public {
		SCYVault nonNative = setupVault(address(underlying), false);
		...
		SCYVault nativeVault = setupVault(address(underlying), true);
	...
	function testMisconfiguration() public {
		...
		vault = setupVault(address(testToken), false);
		scyDeposit(vault, address(this), vault.MIN_LIQUIDITY());
