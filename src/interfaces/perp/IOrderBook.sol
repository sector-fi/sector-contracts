...
interface IOrderBook {
	...
	/// @param fundingGrowthGlobal The funding growth info, detail on `Growth`
	...
	function replaySwap(ReplaySwapParams memory params)
		external
		returns (ReplaySwapResponse memory response);
	...
	function getOpenOrderIds(address trader, address baseToken)
		external
		view
	...
	/// @return info The open order info encoded in `OpenOrderInfo`
	function getOpenOrderById(bytes32 orderId) external view returns (OpenOrderInfo memory info);
	...
	/// @return info he open order info encoded in `OpenOrderInfo`
	function getOpenOrder(
		address trader,
		address baseToken,
		int24 lowerTick,
		int24 upperTick
	) external view returns (OpenOrderInfo memory info);
	...
	function hasOrder(address trader, address[] calldata tokens)
		external
		view
		returns (bool hasOrder);
