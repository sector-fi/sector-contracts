...
import { IVault } from "../../interfaces/perp/IVault.sol";
import { IClearingHouse } from "../../interfaces/perp//IClearingHouse.sol";
import { IMarketRegistry } from "../../interfaces/perp//IMarketRegistry.sol";
import { IAccountBalance } from "../../interfaces/perp//IAccountBalance.sol";
import { IBaseToken } from "../../interfaces/perp//IBaseToken.sol";
import { IClearingHouseConfig } from "../../interfaces/perp/IClearingHouseConfig.sol";
import { IIndexPrice } from "../../interfaces/perp/IIndexPrice.sol";
import { IOrderBook } from "../../interfaces/perp/IOrderBook.sol";
...
contract FungibleVault is ReentrancyGuard, ERC20 {
	...
	function deposit(uint256 amount, address receiver) external nonReentrant returns (uint256) {
		...
		uint256 shares;
		if (totalSupply() == 0) {
			shares = amount; // response.liquidity
		} else {
	...
	function redeem(
		...
		// (should always have 1 order and 0 taker position)
	...
	// TODO: open order execution:
	// 1. remove lp
	// 2. place limit order
	// 3. off-chian bot to adjusts limit order once price move x amount
	// 4. options: off-chain bot places small market order w fixed slippage & updates limit order
