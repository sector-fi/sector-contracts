...
// 		return IERC20(wantToken).balanceOf(address(this)) + totalExecutorFunds() - feesCollected;
// 	}
...
// 			fees += ((duration * managementFee * currentFunds) / MAX_SECONDS) / MAX_BPS;
// 		}
...
// 		require(ITradeExecutor(_tradeExecutor).vault() == address(this), "INVALID_VAULT");
// 		require(IERC20(wantToken).allowance(_tradeExecutor, address(this)) > 0, "NO_ALLOWANCE");
...
// 		(uint256 executorFunds, uint256 blockUpdated) = ITradeExecutor(_tradeExecutor).totalFunds();
...
// 			(uint256 executorFunds, uint256 blockUpdated) = ITradeExecutor(executor).totalFunds();
// 			areFundsUpdated(blockUpdated);
...
// 	event UpdatedGovernance(address indexed oldGovernance, address indexed newGovernance);
...
// 	event UpdatedSlippageAccounter(address indexed oldAccounter, address indexed newAccounter);
...
// 		require(block.number <= _blockUpdated + BLOCK_LIMIT, "FUNDS_NOT_UPDATED");
// 	}
// }
