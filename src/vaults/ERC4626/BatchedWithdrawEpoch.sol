...
abstract contract BatchedWithdrawEpoch is ERC4626 {
	...
	function _redeem(address account) internal returns (uint256 amountOut, uint256 shares) {
		...
		if (withdrawRecord.timestamp >= lastEpochTimestamp) revert NotReady();
	...
	/// TODO dissalow cancel after redeem has been processed?
	function cancelRedeem() public virtual {
		WithdrawRecord storage withdrawRecord = withdrawLedger[msg.sender];
		if (lastEpochTimestamp <= withdrawRecord.timestamp) revert CannotCancelProccesedRedeem();
	...
	function redeemIsReady(address user) external view returns (bool) {
		...
		return lastEpochTimestamp > withdrawRecord.timestamp && withdrawRecord.value > 0;
	}
