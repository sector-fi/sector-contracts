...
contract stETHVault is SCYStrategy, SCYVault {
	...
	function _stratValidate() internal view override {
		if (
			address(underlying) != address(stETH(strategy).underlying()) ||
			yieldToken != address(stETH(strategy).stETH())
		) revert InvalidStrategy();
	}

	function _stratDeposit(uint256 amount) internal override returns (uint256) {
		return stETH(strategy).deposit(amount);
	}
	...
	function _selfBalance(address token) internal view virtual override returns (uint256) {
		if (token == yieldToken) return stETH(strategy).collateralBalance();
